...
" Added RSL new feature and some changes: Mostafa Ari <mostafa.ari@gmail.com>
...
syn keyword slRepeat		illuminance illuminate solar gather
syn keyword slStruct		class struct
...
syn keyword slVariable dtime dPdtime
 ...
 "defined functions
 syntax keyword slFunctions   abs acos ambient area arraylength asin atan atmosphere attribute bake3d calculatenormal capacity caustic ceil cellnoise clamp comp concat cos ctransform degrees depth Deriv determinant diffuse displacement distance Du Dv

 syntax keyword slFunctions   environment exp erf erfc faceforward filterstep floor format fresnel getlight getlights getshader getshaders getvar gridmax gridmin gridpattern hasmethod incident indirectdiffuse length _lightingstart lightsource log

  syntax keyword slFunctions max match min mix mod _needsurface noise normalize ntransform occlusion opposite option phong photonmap pnoise PI  pop pow printf ptlined push ptexture radians random rayinfo reflect refract rendererinfo reserve resize rotate round

   syntax keyword slFunctions setcomp setxcomp setycomp setzcomp shadername shadow sign sin smoothstep solar specular specularbrdf spline sqrt step subsurface surface tan texture texture3d textureinfo trace transform transmission vtransform wnoise xcomp ycomp zcomp
...
if version >= 508 || !exists("did_sl_syntax_inits")
  ...
  HiLink slFunctions    Function
  HiLink slStruct	Structure
